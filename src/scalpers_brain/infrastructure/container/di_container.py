...
class DIContainer:
    ...
    def __init__(self):
        ...
        self.logger.info("DIContainer inicializado")
    ...
    def register_factory(self, service_type: Type[T], factory: Callable[[], T], lifetime: ServiceLifetime = ServiceLifetime.TRANSIENT) -> 'DIContainer':
        ...
        self.logger.info(f"Registrado factory para {service_type.__name__} como {lifetime.value}")
    ...
    def register_instance(self, service_type: Type[T], instance: T) -> 'DIContainer':
        ...
        self.logger.info(f"Registrada instancia para {service_type.__name__}")
    ...
    def _register_service(self, service_type: Type[T], implementation_type: Optional[Type], lifetime: ServiceLifetime) -> 'DIContainer':
        ...
        self.logger.info(
            f"Registrado {service_type.__name__} -> {impl_type.__name__} "
            f"como {lifetime.value} con {len(dependencies)} dependencias"
        )
    ...
    def resolve(self, service_type: Type[T]) -> T:
        ...
        if service_type not in self._services:
            raise ValueError(f"Servicio {service_type.__name__} no está registrado")
        ...
        if service_type in self._building_stack:
            ...
            raise ValueError(f"Dependencia circular detectada: {circular_deps}")
    ...
    def _create_instance(self, descriptor: ServiceDescriptor) -> Any:
        ...
        try:
            ...
            self.logger.debug(f"Creada instancia de {impl_type.__name__}")
            return instance
            
        except Exception as e:
            self.logger.error(f"Error creando instancia de {impl_type.__name__}: {e}")
            raise
        ...
        try:
            ...
            for param_name, param in signature.parameters.items():
                ...
                if param.annotation != inspect.Parameter.empty:
                    ...
                    self.logger.warning(
                        f"Parámetro '{param_name}' en {impl_type.__name__}.__init__ "
                        f"no tiene anotación de tipo"
                    )
            
            return dependencies
            
        except Exception as e:
            self.logger.error(f"Error analizando dependencias para {impl_type.__name__}: {e}")
            return []
    ...
    def clear_scoped(self):
        ...
        self.logger.debug("Instancias scoped eliminadas")
...
def auto_register(container: DIContainer, module_or_classes: Union[Any, List[Type]]):
    ...
    for cls in classes_to_register:
        if hasattr(cls, '_di_lifetime'):
            ...
            logging.getLogger("auto_register").info(f"Auto-registrado {cls.__name__} como {lifetime.value}")
