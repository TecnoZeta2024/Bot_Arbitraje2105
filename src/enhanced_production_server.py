"""
Enhanced Production Server - Bot_Arbitraje2105
Servidor mejorado con reconexión automática y detección dinámica de puertos
"""
...
def find_available_port(start_port: int = 8501, max_attempts: int = 10) -> int:
    ...
    raise RuntimeError(f"No se pudo encontrar un puerto disponible entre {start_port} y {start_port + max_attempts - 1}")
...
class ServerConfig:
    """Configuración del servidor con auto-discovery"""
    def __init__(self):
        self.host = "127.0.0.1"
        self.preferred_port = 8501
    ...
    def get_available_port(self) -> int:
        ...
        if not self.current_port:
            self.current_port = find_available_port(self.preferred_port, self.max_port_attempts)
        return self.current_port
    
    def reset_port(self):
        """Resetea el puerto para buscar uno nuevo"""
        self.current_port = None
...
# ==================== FASTAPI APPLICATION ====================
app = FastAPI(
    title="Bot Arbitraje Enhanced Production Server",
    description="Servidor de producción mejorado con reconexión automática",
    version="3.1.0"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://localhost:3000", "http://localhost:3003", "http://localhost:8501", "http://localhost:8502"],
...
async def root():
    ...
    return {
        "name": "Bot Arbitraje Enhanced Production Server",
        "version": "3.1.0",
        "status": "operational",
        "current_port": server_config.current_port,
        "features": [
...
async def health_check():
    ...
    return {
        ...
        "server": {
            ...
            "port": server_config.current_port,
        ...
        "services": {
            ...
            "system_monitor_active": system_monitor.is_running,
        }
    }
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await manager.send_personal_message({
            "type": "connection_ack",
            "client_id": client_id,
            "message": "Connected to enhanced production server",
...
async def startup_event():
    ...
    logger.info("=" * 80)
    logger.info("[LAUNCH] ENHANCED PRODUCTION SERVER STARTING")
    logger.info("=" * 80)
    
    # Añadir el handler de logging de WebSocket
    websocket_log_handler = WebSocketLoggingHandler(manager)
    websocket_log_handler.setLevel(logging.WARNING)
...
async def shutdown_event():
    ...
    logger.info("Shutting down enhanced production server...")
            ...
            if len(manager.active_connections) > 0:
                await manager.broadcast({
                    ...
                    "data": {
                        ...
                        "timestamp": time.time(),
                        "system_monitor_status": system_status_data,
...
def start_production_server_with_retry():
    """Inicia el servidor con reintentos automáticos"""
    attempt = 1
    
    while attempt <= server_config.max_restart_attempts:
        try:
            # Obtener puerto disponible
            port = server_config.get_available_port()
            host = server_config.host
            
            print("\n" + "="*80)
            print(f"[LAUNCH] BOT ARBITRAJE ENHANCED SERVER v3.1.0 - Attempt {attempt}")
            print("="*80)
            print(f"[API] Server: http://{host}:{port}")
            print(f"[WS]  WebSocket: ws://{host}:{port}/ws")
            print("="*80)
            print("[FEATURES]")
            print("   🔄 Automatic port discovery")
            print("   🔁 Robust reconnection handling")
            print("   📊 Real-time market data")
            print("   💓 WebSocket heartbeat")
            print("   🎯 Trading management")
            print("   📝 Paper trading")
            print("   🤖 AI signals")
            print("   💼 Portfolio tracking")
            print("="*80)
            print(f"[LOGS] production_server.log")
            ...
            logger.error(f"Server startup failed (attempt {attempt}/{server_config.max_restart_attempts}): {e}")
            
            if attempt < server_config.max_restart_attempts:
                logger.info(f"Waiting {server_config.restart_delay} seconds before retry...")
                time.sleep(server_config.restart_delay)
                
                # Resetear puerto para buscar uno nuevo
                server_config.reset_port()
                attempt += 1
            else:
...
def start_production_server():
    ...
    start_production_server_with_retry()
